# Deep learning z-scores 
# Test combine 
# Test seperately at lower part 
...
X_train_all = train_data_mat_all[,-55]
X_test_all = test_data_mat_all[,-55]

Y_train_all = train_data_mat_all[,55]
Y_test_all = test_data_mat_all[,55]
...
# dim(X_train_all)   # 9234 * 54 variables
# dim(X_test_all)    # 3956 * 54 variables 
...
model %>%
  layer_dense(units=54, activation = 'relu', input_shape = ncol(X_train_all)) %>%
...
# Data partition, train-test split
X_test_stemi_all = test_stemi_data_mat_all[,-55]
X_test_nstemi_all = test_nstemi_data_mat_all[,-55]


Y_test_stemi_all = test_stemi_data_mat_all[,55]
Y_test_nstemi_all = test_nstemi_data_mat_all[,55]
...
all_roc
ci.auc(as.vector(Y_test_stemi_label_all), as.vector(all_prob))  # 95% CI: 0.9429-0.9539 (DeLong)
...
## Plot roc 
all_roc <- roc(as.vector(Y_test_nstemi_label_all), as.vector(all_prob), positive = 1, type ="prob") 
all_roc
ci.auc(as.vector(Y_test_nstemi_label_all), as.vector(all_prob))  # 95% CI: 0.9529-0.9647 (DeLong)
